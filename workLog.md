...
## 6 月 4日
- 阶段性成果：已经不存在程序上的例外，对于随机生成的OCA可以生成表示可达关系的QFPA公式
- 需要开始论文的写作
...
- SETTA ddl: 6月21日
- TODOs:
1. 对几个典型的例子进行测试，保证生成公式的正确性，如果不正确需要继续debug.
2. 开始写论文，目前的问题：
   - SWG的定义如果按照老师的定义可能会出现环，但我的定义又比较繁琐。 
   - 证明如果存在长度大于 $2|V|^2 + 1 $ 的时候肯定存在长度等于 $2|V|^2 + 1 $ 的可达证书


## 8 月 14 日

记录： 改变计数器自动机每个 transition 对计数器值的改变的绝对值得最大值，将其变为 m
需要改变的几点：
- 中间节点对应的变量的bound：$z_{mid} \ge m\cdot |V|$
- 满足上述条件的中间节点的可能的计数器的最大值：$2m\cdot |V|$
- 可能的环权重的最大负值：$-1$
- 最后动态规划需要考虑的长度：$2m\cdot|V|^2 + 1$
